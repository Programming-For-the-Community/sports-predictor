...
| Player stat props -- one model per stat (e.g. `passing_yards`, `rushing_yards`, `receiving_yards`, `defensive_sacks`) | XGBoost regression (`train_player_prop_model.py`) -- one script, not one script per stat; which stat to train is set by the `TARGET_STAT` environment variable at `aws ecs run-task` time (see `Terraform/ecs-task-nfl-train-player-prop-model.tf`). The dataset is filtered to rows where the player recorded `TARGET_STAT` in the game being labeled, has at least `MIN_PRIOR_GAMES_WITH_STAT` prior games with it (`games_with_<stat>`), AND has an `avg_<stat>` at least `MIN_AVG_FRACTION_OF_MEDIAN` of the remaining population's own median -- the first two filters catch a true one-off (a WR's single career pass attempt), the third catches a player who clears that bar by recurring at trivial volume instead (e.g. a real but minor wildcat passing package) rather than being a genuine participant in this stat category | That player's own rolling per-stat averages (`avg_<stat>`), `games_with_<stat>`, `games_played`, `starts`, plus event context reoriented to the player's own/opponent perspective (`is_home`, `week`, `season_type`, `venue_indoor`, `weather_temperature`, `rest_days`, `own_elo`/`opponent_elo`/`elo_diff`, `is_divisional_game`, `is_international_game`, `travel_km` -- see "Event context" above) -- restricted to columns with at least one non-null value in the filtered population, since `player_features.parquet`'s schema spans every position in the league and most are structurally inapplicable to any single stat's population |
