...
# 15 minutes catches an early kickoff without waiting on the scheduled
...
def refresh(storage, s3, bucket: str, client, sport: str) -> dict:
    ...
    scores.tf. Reads already-ingested events from DynamoDB and only
    reaches out to ESPN once _candidate_events finds something worth
    checking.
    ...
    only re-fetches this sport once a day (scheduler-ingest-orchestrator.
    tf), so DynamoDB's own event status can lag the real result by up to
    24h. Until that catches up (this event drops out of
    get_all_events(status="scheduled") on its own once it does), this
    cache is the only place the frontend can still get the game's final
    score/stats from -- without carrying it forward, "completed" state was
    being overwritten out of the cache on the very next tick, ~60s after
    the game actually ended, and the event reverted to looking unplayed."""
...
def get_live_scores(s3, bucket: str) -> dict:
    """Called by GET /ncaafb/live-scores (handler.py). Returns an empty
    events dict rather than an error on a cache miss or a stale cache."""
