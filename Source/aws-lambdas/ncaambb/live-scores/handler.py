"""
NCAA MBB live-score cache Lambda. Two distinct triggers, one function:

  - EventBridge Scheduler (scheduler-ncaambb-live-scores.tf), every 60s,
    event shape {"detail-type": "LiveScoreRefresh"}.
  - API Gateway (REST API, Lambda proxy integration), GET /ncaambb/live-
    scores, behind the same Cognito authorizer as every other NCAA MBB
    route.
