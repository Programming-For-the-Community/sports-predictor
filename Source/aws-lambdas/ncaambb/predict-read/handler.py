"""
NCAA MBB read-only serving Lambda. Triggered by API Gateway behind the
Cognito authorizer.

Routes:
    GET /ncaambb/events?status=scheduled|completed
    GET /ncaambb/models
    GET /ncaambb/season
    GET /ncaambb/predictions/events/{event_id}
    GET /ncaambb/predictions/events/{event_id}/players/{entity_id}?stat=points

See library.serving.ncaambb_reads for events/models/season. The two
...
from library.serving.ncaambb_reads import get_season_projection, list_events, list_models
...
logger = logging.getLogger("ncaambb-predict-read")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if resource == "/ncaambb/predictions/events/{event_id}":
        ...
        if resource == "/ncaambb/predictions/events/{event_id}/players/{entity_id}":
