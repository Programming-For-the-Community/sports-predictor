...
def seed_teams(client: NBAClient, storage: PipelineStorage) -> None:
    espn_backfill_common.seed_teams(client, storage, normalize, "nba", logger)


def process_game(client: NBAClient, storage: PipelineStorage, season: int, event_id: str) -> None:
    espn_backfill_common.process_game(client, storage, normalize, "nba", season, event_id, logger)
