"""
Shared test fixtures for test_ingest_*.py -- not test_-prefixed, so
...
def scoreboard(events: list, week: int = 5, season_year: int = SEASON_YEAR, season_type: int = SEASON_TYPE) -> dict:
    """Matches site.web.api.espn.com's real shape: no top-level "season"
    key; year/type live under leagues[0].season, and type is itself a
    dict, not a bare int. week stays top-level."""
    return {
        "week": {"number": week},
...
def teams_response(*team_ids: str) -> dict:
    """Matches NFLClient.get_teams' real shape:
...
def make_client(board: dict, summary: dict | None = None):
    ...
    # they just need it to run without error. test_ingest_rosters.py sets
    # this explicitly.
