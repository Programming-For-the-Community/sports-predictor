...
vs. API Gateway's GET /nfl/live-scores) and response shaping/error
handling. live_scores.py's own logic is covered directly in
test_live_scores.py -- this file only verifies routing.

The nfl_live_scores module is registered in sys.modules by conftest.py.
"""
import json
from unittest.mock import MagicMock, patch

import live_scores
import nfl_live_scores

# nfl_live_scores's own _storage singleton is reset before/after every
# test in this directory by conftest.py's own _reset_nfl_singletons
# fixture.
...
class TestDispatch:
    def test_refresh_trigger_calls_live_scores_refresh_not_the_api_route(self):
        nfl_live_scores._storage = MagicMock()
        with patch.object(live_scores, "refresh", return_value={"polled": 1}) as mock_refresh:
            result = nfl_live_scores.lambda_handler({"detail-type": "LiveScoreRefresh"}, None)
    ...
    def test_live_scores_route_returns_cached_events(self):
        ...
        with patch.object(live_scores, "get_live_scores", return_value={"events": {"1": {"live": True}}}):
            response = nfl_live_scores.lambda_handler(_api_event("/nfl/live-scores"), None)

        assert response["statusCode"] == 200
        assert json.loads(response["body"]) == {"events": {"1": {"live": True}}}

    def test_unknown_resource_returns_404(self):
        response = nfl_live_scores.lambda_handler(_api_event("/nfl/nonsense"), None)
