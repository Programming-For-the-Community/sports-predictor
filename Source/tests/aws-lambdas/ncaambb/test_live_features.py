"""
Unit tests for ncaambb/predict/live_features.py. NCAA MBB's
build_event_features carries no leader-tracking sub-feature (see
library.features.ncaambb's own docstring for why), so
...
def _event(event_key, event_date, home_id, away_id, season=2026, home_score=None, away_score=None):
...
def _player_game(entity_id, team_id, event_key, event_date, stat_line):
    return {
        ...
        "event_date": event_date, "player_key": f"SPORT#NCAAMBB#PLAYER#{entity_id}", "stat_line": stat_line,
    }
...
class TestBoxScoreCandidateIds:
    ...
    def test_returns_still_rostered_candidates_credited_with_the_stat(self):
        storage = MagicMock()
        storage.get_team_events.return_value = [_event("e1", "2026-01-11", "13", "2")]
        storage.get_player_game_stats_for_event.return_value = [
            _player_game("101", "13", "e1", "2026-01-11", {"points": 28}),
            _player_game("102", "13", "e1", "2026-01-11", {"points": 12}),
        ]
        storage.get_entity.side_effect = lambda sport, entity_id, entity_type: {
            "101": _entity("13"), "102": _entity("99"),  # 102 has since transferred
        }[entity_id]

        ids = live_features._box_score_candidate_ids(storage, "ncaambb", "13", "2026-01-18", 2026, "points")
    ...
    def test_skips_a_row_missing_the_stat_key(self):
        ...
        storage.get_team_events.return_value = [_event("e1", "2026-01-11", "13", "2")]
        storage.get_player_game_stats_for_event.return_value = [
            _player_game("101", "13", "e1", "2026-01-11", {"assists": 6}),
        ]
        storage.get_entity.return_value = _entity("13")
    ...
    def test_a_candidate_appearing_in_multiple_games_is_only_checked_once(self):
        storage = MagicMock()
        storage.get_team_events.return_value = [
            _event("e1", "2026-01-11", "13", "2"),
            _event("e2", "2026-01-09", "13", "17"),
        ]
        storage.get_player_game_stats_for_event.side_effect = lambda event_key: {
            "e1": [_player_game("101", "13", "e1", "2026-01-11", {"points": 28})],
            "e2": [_player_game("101", "13", "e2", "2026-01-09", {"points": 30})],
        }[event_key]
        storage.get_entity.return_value = _entity("13")
    ...
    def test_stops_at_the_season_lookback_bound(self):
        storage = MagicMock()
        too_old = _event("e0", "2024-01-01", "13", "2", season=2024)
...
class TestBuildLiveEventFeatures:
    def test_raises_when_event_not_found(self):
        ...
        with pytest.raises(live_features.EventNotFoundError):
            live_features.build_live_event_features(storage, "ncaambb", "SPORT#NCAAMBB#EVENT#1")

    def test_raises_when_missing_home_or_away(self):
        storage = MagicMock()
        storage.get_event.return_value = {
            "event_key": "SPORT#NCAAMBB#EVENT#1", "event_date": "2026-01-11",
    ...
    def test_builds_a_feature_row_with_no_history(self):
        ...
        row = live_features.build_live_event_features(storage, "ncaambb", event["event_key"])
...
class TestBuildLivePlayerFeatures:
    def test_raises_when_event_not_found(self):
        ...
        storage.get_event.return_value = None
        with pytest.raises(live_features.EventNotFoundError):
            live_features.build_live_player_features(storage, "ncaambb", "SPORT#NCAAMBB#EVENT#1", "101")

    def test_raises_when_entity_not_found(self):
        storage = MagicMock()
        storage.get_event.return_value = _event("SPORT#NCAAMBB#EVENT#1", "2026-01-11", "13", "2")
        storage.get_entity.return_value = None
        with pytest.raises(live_features.EventNotFoundError):
            live_features.build_live_player_features(storage, "ncaambb", "SPORT#NCAAMBB#EVENT#1", "101")
    ...
    def test_builds_a_feature_row(self):
        ...
        row = live_features.build_live_player_features(storage, "ncaambb", event["event_key"], "101")
...
class TestBuildLiveEventLeaderCandidates:
    def test_raises_when_event_not_found(self):
        ...
        storage.get_event.return_value = None
        with pytest.raises(live_features.EventNotFoundError):
            live_features.build_live_event_leader_candidates(storage, "ncaambb", "SPORT#NCAAMBB#EVENT#1")
    ...
    def test_shape_has_three_categories_and_empty_lists_when_no_candidates(self):
        ...
        result = live_features.build_live_event_leader_candidates(storage, "ncaambb", event["event_key"])
    ...
    def test_ranks_multiple_scoring_candidates_by_recent_volume(self):
        storage = MagicMock()
        event = _event("SPORT#NCAAMBB#EVENT#1", "2026-01-11", "13", "2")
        past_game = _event("SPORT#NCAAMBB#EVENT#900", "2026-01-09", "13", "17")
        storage.get_event.return_value = event
        storage.get_all_events.return_value = []
        ...
        storage.get_player_game_stats_for_event.return_value = [
            _player_game("101", "13", past_game["event_key"], "2026-01-09", {"points": 8}),
            _player_game("102", "13", past_game["event_key"], "2026-01-09", {"points": 30}),
            _player_game("103", "13", past_game["event_key"], "2026-01-09", {"points": 18}),
            _player_game("104", "13", past_game["event_key"], "2026-01-09", {"points": 22}),
            _player_game("105", "13", past_game["event_key"], "2026-01-09", {"points": 15}),
            _player_game("106", "13", past_game["event_key"], "2026-01-09", {"points": 4}),
        ]
        storage.get_entity.return_value = _entity("13")
