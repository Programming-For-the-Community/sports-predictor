"""
Unit tests for ncaambb/predict/event_prediction.py -- predict_event,
...
class TestReconcileScores:
    def test_preserves_margin_exactly(self):
        result = event_prediction.reconcile_scores(margin=7.0, home_score=72.0, away_score=65.0)
        assert result["home_score"] - result["away_score"] == pytest.approx(7.0)

    def test_preserves_combined_total(self):
        result = event_prediction.reconcile_scores(margin=3.0, home_score=70.0, away_score=67.0)
        assert result["home_score"] + result["away_score"] == pytest.approx(137.0)
...
class TestPredictEvent:
    def test_predicts_win_probability_and_scores(self):
        ...
        with patch.object(event_prediction.live_features, "build_live_event_features", return_value={"f": 1}), \
            ...
            mock_predict.side_effect = [0.65, 3.0, 72.0, 69.0]  # win_prob, margin, home_score, away_score
...
class TestPredictEventLeaders:
    def test_returns_none_on_candidate_build_failure(self):
        ...
        with patch.object(event_prediction.live_features, "build_live_event_leader_candidates", side_effect=Exception("boom")):
            result = event_prediction.predict_event_leaders(storage, s3, predictions_table, "SPORT#NCAAMBB#EVENT#1")
    ...
    def test_scores_every_candidate_in_a_category(self):
        ...
        storage.get_entity.return_value = {"name": "Cooper Flagg"}
        ...
        assert home_scoring[0]["name"] == "Cooper Flagg"
    ...
    def test_reorders_candidates_by_their_own_predicted_value_not_arrival_order(self):
        ...
        # output order must follow the prediction, not the input order.
        storage, s3, predictions_table = MagicMock(), MagicMock(), MagicMock()
        candidates = {
            "home": {**_EMPTY_TEAM_CANDIDATES, "scoring": [{"entity_id": "101"}, {"entity_id": "102"}]},
            "away": _EMPTY_TEAM_CANDIDATES,
        ...
        with patch.object(event_prediction.live_features, "build_live_event_leader_candidates", return_value=candidates), \
             patch.object(event_prediction.model_loader, "load_current_model", return_value=(MagicMock(), {"version": 1})), \
             patch.object(event_prediction.model_loader, "predict", side_effect=[14.0, 26.0]):
            result = event_prediction.predict_event_leaders(storage, s3, predictions_table, "SPORT#NCAAMBB#EVENT#1")

        assert [row["entity_id"] for row in result["home"]["scoring"]] == ["102", "101"]
    ...
    def test_a_candidate_missing_its_stat_sorts_last_rather_than_erroring(self):
        ...
        with patch.object(event_prediction.live_features, "build_live_event_leader_candidates", return_value=candidates), \
             ...
             patch.object(event_prediction.model_loader, "predict", return_value=18.0):
            result = event_prediction.predict_event_leaders(storage, s3, predictions_table, "SPORT#NCAAMBB#EVENT#1")
...
class TestComputeAndCacheEvent:
    ...
    RESULT = {
        "event_key": "SPORT#NCAAMBB#EVENT#1",
        "predictions": {
            "win_probability": {"home_win_probability": 0.6, "model_version": 1},
            "margin": {"value": 3.0, "model_version": 2},
            "home_score": {"value": 72.0, "model_version": 3},
            "away_score": {"value": 69.0, "model_version": 4},
        },
        "leaders": None,
    }
