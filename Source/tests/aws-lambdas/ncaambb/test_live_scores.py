...
lookup -- NCAA MBB's data source and live-scores source are both ESPN, so
...
def _espn_summary(event_id: str) -> dict:
    # Basketball's player-level stat block has no "name" field at all (see
        ...
        "boxscore": {
            "players": [{
                ...
                "statistics": [{
                    ...
                    "athletes": [{
                        "athlete": {"id": "100", "displayName": "Player One", "jersey": "9", "position": {"abbreviation": "G"}},
                        "stats": ["27", "5", "8"],
                    }],
                }],
            }],
...
class TestExtractLiveState:
    def test_parses_a_live_game(self):
        espn_event = _espn_event("1", state="in", completed=False, detail="2nd 12:04", home_score="44", away_score="41")

        state = live_scores._extract_live_state(espn_event)

        assert state == {"live": True, "completed": False, "detail": "2nd 12:04", "home_score": 44, "away_score": 41}
    ...
    def test_a_final_game_is_completed_not_live(self):
        espn_event = _espn_event("1", state="post", completed=True, detail="Final", home_score="72", away_score="65")
...
class TestRefresh:
    ...
    def test_a_live_candidate_gets_fetched_and_cached(self):
        ...
        client.get_scoreboard_for_date.return_value = {
            "events": [_espn_event("1", state="in", completed=False, detail="2nd", home_score="40", away_score="35")],
        }
        s3 = _make_s3()
        ...
        assert cached["events"]["1"]["home_score"] == 40
    ...
    def test_an_event_confirmed_completed_last_cycle_is_not_repolled(self):
        ...
        s3._store[live_scores.LIVE_SCORES_CACHE_KEY] = json.dumps({
            ...
            "events": {"1": {"live": False, "completed": True, "detail": "Final", "home_score": 70, "away_score": 61}},
        }).encode("utf-8")
    ...
    def test_a_completed_events_final_state_is_carried_forward_not_dropped(self):
        ...
        assert cached["events"]["1"] == {
            "live": False, "completed": True, "detail": "Final", "home_score": 70, "away_score": 61,
        }
...
class TestLivePlayerStats:
    def test_a_live_candidates_boxscore_is_fetched_and_cached(self):
        ...
        cached = json.loads(s3._store[live_scores.LIVE_SCORES_CACHE_KEY])
        assert cached["events"]["1"]["player_stats"]["100"] == {"points": 27, "rebounds": 5, "assists": 8}
...
class TestGetLiveScores:
    ...
    def test_returns_cached_events_when_fresh(self):
        ...
        s3._store[live_scores.LIVE_SCORES_CACHE_KEY] = json.dumps({
            "fetched_at": datetime.now(timezone.utc).isoformat(),
            "events": {"1": {"live": True, "completed": False, "detail": "2nd", "home_score": 40, "away_score": 35}},
        }).encode("utf-8")
