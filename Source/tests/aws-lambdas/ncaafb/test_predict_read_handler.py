...
Source/tests/library/serving/test_ncaafb_reads.py, and the pure cache-
...
class TestPredictionRoutes:
    EVENT_ID = "401520281"
    EVENT_KEY = build_event_key("ncaafb", EVENT_ID)
    CACHE_KEY = f"predictions-cache/ncaafb/events/{EVENT_KEY}.json"
    EVENT_RESOURCE = "/ncaafb/predictions/events/{event_id}"
    ...
    def test_fresh_cache_hit_returns_the_cached_result_without_triggering_compute(self):
        ...
        with patch.object(ncaafb_predict_read, "_get_model_bucket", return_value=s3), \
             patch.object(ncaafb_predict_read, "_get_predict_invoker") as get_invoker:
            response = ncaafb_predict_read.lambda_handler(
    ...
    def test_stale_cache_hit_returns_203_with_the_cached_result_and_triggers_a_refresh(self):
        ...
        with patch.object(ncaafb_predict_read, "_get_model_bucket", return_value=s3), \
             patch.object(ncaafb_predict_read, "_get_predict_invoker", return_value=invoker):
            response = ncaafb_predict_read.lambda_handler(
                _predict_event(self.EVENT_RESOURCE, {"event_id": self.EVENT_ID}), None,
            )
    ...
    def test_a_repromoted_model_makes_a_completed_events_own_cache_stale_too(self):
        ...
        current = _model_version_state("ncaafb", {"win_probability": 2, "margin": 1, "home_score": 1, "away_score": 1})
        state = dict(current)
        state[self.CACHE_KEY] = {
    ...
    def test_player_prop_route_fresh_cache_hit(self):
        stat = "passing_yards"
        cache_key = f"predictions-cache/ncaafb/events/{self.EVENT_KEY}/players/101/{stat}.json"
        state = {
            current_version_key("ncaafb", "player-prop-passing-yards"): {"version": 2},
