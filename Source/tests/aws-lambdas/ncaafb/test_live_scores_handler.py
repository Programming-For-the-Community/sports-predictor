...
import live_scores
import ncaafb_live_scores

# ncaafb_live_scores's own _storage singleton is reset before/after every
...
class TestDispatch:
    def test_refresh_trigger_calls_live_scores_refresh_not_the_api_route(self):
        ncaafb_live_scores._storage = MagicMock()
        with patch.object(live_scores, "refresh", return_value={"polled": 1}) as mock_refresh:
            result = ncaafb_live_scores.lambda_handler({"detail-type": "LiveScoreRefresh"}, None)
    ...
    def test_live_scores_route_returns_cached_events(self):
        ...
        with patch.object(live_scores, "get_live_scores", return_value={"events": {"1": {"live": True}}}):
            response = ncaafb_live_scores.lambda_handler(_api_event("/ncaafb/live-scores"), None)

        assert response["statusCode"] == 200
        assert json.loads(response["body"]) == {"events": {"1": {"live": True}}}

    def test_unknown_resource_returns_404(self):
        response = ncaafb_live_scores.lambda_handler(_api_event("/ncaafb/nonsense"), None)
