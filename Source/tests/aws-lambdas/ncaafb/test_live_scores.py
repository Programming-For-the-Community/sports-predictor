...
class TestCandidateEvents:
    def test_excludes_an_event_more_than_15_minutes_before_kickoff(self):
        ...
        candidates = live_scores._candidate_events([_event("1", "2026-09-14T17:16:00Z")], now, set())
    ...
    def test_includes_an_event_within_15_minutes_of_kickoff(self):
        ...
        candidates = live_scores._candidate_events([_event("1", "2026-09-14T17:14:00Z")], now, set())
    ...
    def test_excludes_an_event_past_the_safety_cap(self):
        ...
        candidates = live_scores._candidate_events([_event("1", "2026-09-14T09:00:00Z")], now, set())  # 8h before now
    ...
    def test_excludes_an_event_already_confirmed_completed_by_a_prior_cycle(self):
        ...
        candidates = live_scores._candidate_events([_event("1", "2026-09-14T17:00:00Z")], now, already_completed={"1"})
...
class TestExtractLiveState:
    def test_parses_a_live_game(self):
        espn_event = _espn_event("1", state="in", completed=False, detail="Q3", home_score="24", away_score="17")

        state = live_scores._extract_live_state(espn_event)

        assert state == {"live": True, "completed": False, "detail": "Q3", "home_score": 24, "away_score": 17}
    ...
    def test_a_final_game_is_completed_not_live(self):
        espn_event = _espn_event("1", state="post", completed=True, detail="Final", home_score="31", away_score="24")
...
class TestRefresh:
    ...
    def test_a_completed_events_final_state_is_carried_forward_not_dropped(self):
        ...
        # after the game ended -- long before the once-daily ingest could
        # flip its real DynamoDB status to "completed". The frontend then
        # had nowhere to get the final score/FINAL state from and rendered
        # the game as if it hadn't been played yet.
        storage = MagicMock()
        storage.get_all_events.return_value = [_event("1", datetime.now(timezone.utc).isoformat())]
        client = MagicMock()
        s3 = _make_s3()
        ...
        assert cached["events"]["1"] == {
            "live": False, "completed": True, "detail": "Final", "home_score": 20, "away_score": 14,
        }
    ...
    def test_a_completed_event_no_longer_scheduled_stops_being_carried_forward(self):
        # Once the once-daily ingest catches up and flips this event's own
        # DynamoDB status to "completed", get_all_events(status="scheduled")
        # stops returning it -- the frontend now gets its final state from
        # the event record itself (status/predictionComparison), so the
...
class TestLivePlayerStats:
    ...
    def test_the_transition_tick_carries_forward_the_last_live_player_stats(self):
        ...
        client = MagicMock()
        client.get_scoreboard_for_date.return_value = {
            "events": [_espn_event("1", state="post", completed=True, detail="Final", home_score="24", away_score="20")],
        }
        s3 = _make_s3()
        s3._store[live_scores.LIVE_SCORES_CACHE_KEY] = json.dumps({
            "fetched_at": datetime.now(timezone.utc).isoformat(),
            "events": {"1": {
                "live": True, "completed": False, "detail": "Q4 00:12", "home_score": 24, "away_score": 20,
                "player_stats": {"100": {"passing_yards": 310}},
            }},
        }).encode("utf-8")
        ...
        assert cached["events"]["1"]["player_stats"] == {"100": {"passing_yards": 310}}
