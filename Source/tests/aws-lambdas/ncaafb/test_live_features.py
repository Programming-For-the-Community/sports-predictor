...
def _entity(team_id, team_id_as_of=None):
    ...
    # reference_date through _still_on_team/_presumptive_leader/
    # _box_score_candidate_ids pass a matching team_id_as_of explicitly;
    # everything else (including callers that don't thread reference_date
    # at all, like build_live_event_leader_candidates) still sees a
    # freshly-confirmed entity regardless of real-world "today".
...
class TestStillOnTeam:
    def test_true_when_entity_team_matches(self):
        storage = MagicMock()
        storage.get_entity.return_value = _entity("61", team_id_as_of="2025-10-18")
        assert live_features._still_on_team(storage, "ncaafb", "101", "61", "2025-10-18") is True

    def test_false_when_entity_team_differs(self):
        storage = MagicMock()
        storage.get_entity.return_value = _entity("52", team_id_as_of="2025-10-18")
        assert live_features._still_on_team(storage, "ncaafb", "101", "61", "2025-10-18") is False
    ...
    def test_false_when_team_id_as_of_is_stale(self):
        # Team matches, but this player hasn't been reconfirmed there
        # (no box score appearance, no roster re-fetch) in
        # _ROSTER_STALENESS_DAYS -- they've likely left the program even
        # though nothing ever explicitly removed them.
        storage = MagicMock()
        storage.get_entity.return_value = _entity("61", team_id_as_of="2025-06-01")
        assert live_features._still_on_team(storage, "ncaafb", "101", "61", "2025-10-18") is False

    def test_false_when_team_id_as_of_is_missing(self):
        storage = MagicMock()
        storage.get_entity.return_value = {"metadata": {"team_id": "61"}}
        assert live_features._still_on_team(storage, "ncaafb", "101", "61", "2025-10-18") is False

    def test_true_at_the_staleness_boundary(self):
        storage = MagicMock()
        storage.get_entity.return_value = _entity("61", team_id_as_of="2025-09-03")  # exactly 45 days before
        assert live_features._still_on_team(storage, "ncaafb", "101", "61", "2025-10-18") is True

    def test_false_one_day_past_the_staleness_boundary(self):
        storage = MagicMock()
        storage.get_entity.return_value = _entity("61", team_id_as_of="2025-09-02")  # 46 days before
        assert live_features._still_on_team(storage, "ncaafb", "101", "61", "2025-10-18") is False
...
class TestPresumptiveLeader:
    ...
    def test_crosses_season_boundary_for_week_0_target(self):
        ...
        storage.get_entity.return_value = _entity("61", team_id_as_of="2026-08-30")
        storage.get_player_game_stats.return_value = [{"entity_id": "101"}]

        result = live_features._presumptive_leader(
            storage, "ncaafb", "61", "2026-08-30", 2026, "passing", 5, reference_date="2026-08-30",
        )
