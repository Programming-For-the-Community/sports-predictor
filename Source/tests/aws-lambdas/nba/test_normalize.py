"""
Unit tests for the NBA normalize Lambda handler. All AWS calls are
...
handler is resilient to individual record failures. NBA's normalizers
are the shared library.normalize.espn functions (see handler.py's own
docstring for why there's no separate library/normalize/nba.py) -- these
tests exercise the dispatch/wiring, not the normalizer logic itself,
which is covered by tests/library/normalize/test_espn_*.py.

The nba_normalize module is registered in sys.modules by conftest.py.
"""
import json
...
class TestDispatch:
    def test_routes_teams_key_to_teams_processor(self):
        ...
        with patch.object(nba_normalize, "_s3", mock_s3), \
            ...
            nba_normalize._dispatch("test-bucket", "nba/teams.json")
    ...
    def test_routes_boxscore_key_to_boxscore_processor(self):
        ...
        with patch.object(nba_normalize, "_s3", mock_s3), \
             patch("nba_normalize.PipelineStorage", return_value=mock_storage):
            nba_normalize._dispatch("test-bucket", "nba/boxscore/2026/1.json")
    ...
    def test_routes_roster_key_to_roster_processor(self):
        ...
        with patch.object(nba_normalize, "_s3", mock_s3), \
             patch("nba_normalize.PipelineStorage", return_value=mock_storage):
            nba_normalize._dispatch("test-bucket", "nba/roster/1.json")
    ...
    def test_unrecognized_key_is_skipped_without_raising(self):
        ...
        with patch.object(nba_normalize, "_s3", mock_s3), \
             patch("nba_normalize.PipelineStorage"):
            nba_normalize._dispatch("test-bucket", "nba/unknown/thing.json")  # must not raise


class TestClearDepartedPlayers:
    def test_clears_a_player_missing_from_the_fresh_roster(self):
        # Regression: a player who's since retired, been waived, or
        ...
        storage.get_team_entities.return_value = [
            ...
            {"entity_id": "a2", "name": "Departed Player", "team_key": "SPORT#NBA#TEAM#1",
             "metadata": {"team_id": "1", "team_id_as_of": "2026-08-12", "position": "F"}},
        ]
        storage.upsert_player_entity.return_value = True
        entities = [{"entity_id": "a1", "metadata": {"team_id": "1"}}]  # only a1 is in the fresh roster

        cleared = nba_normalize._clear_departed_players(storage, "1", entities, "2026-09-06")
    ...
    def test_a_genuinely_empty_fetch_is_left_alone_rather_than_wiping_the_whole_team(self):
        ...
        # than every player on a real roster leaving at once, so an
        # empty fetch is a no-op that self-heals on the next daily
        # re-fetch instead of wiping the team's whole roster attribution
        # over one bad response.
        storage = MagicMock()

        cleared = nba_normalize._clear_departed_players(storage, "1", [], "2026-09-06")
...
class TestLambdaHandler:
    def test_processes_every_record_independently(self):
        ...
        event = {"Records": [
            _s3_record("test-bucket", "nba/scoreboard/20260114.json"),
            _s3_record("test-bucket", "nba/scoreboard/20260115.json"),
        ]}
