"""
Unit tests for nba/predict-read/handler.py -- routing only; the actual
request-shaping logic (list_events/list_models) is exercised in
Source/tests/library/serving/test_nba_reads.py, and the pure cache-
...
class TestWarmup:
    def test_warmup_ping_touches_singletons_and_skips_routing(self):
        with patch.object(nba_predict_read, "_get_storage") as mock_storage, \
             patch.object(nba_predict_read, "_get_model_bucket") as mock_bucket, \
             patch.object(nba_predict_read, "_get_predictions_table") as mock_table:
            response = nba_predict_read.lambda_handler({"warmup": True}, None)
...
class TestRouting:
    def test_events_route(self):
        with patch.object(nba_predict_read, "_get_storage"), \
             patch.object(nba_predict_read, "_get_predictions_table"), \
             patch.object(nba_predict_read, "list_events", return_value={"sport": "nba", "events": []}) as mock_list:
            response = nba_predict_read.lambda_handler(_api_event("/nba/events", {"status": "completed"}), None)

        assert response["statusCode"] == 200
        assert json.loads(response["body"]) == {"sport": "nba", "events": []}
        assert mock_list.call_args.args[-1] == "completed"

    def test_events_route_defaults_status_to_scheduled(self):
        with patch.object(nba_predict_read, "_get_storage"), \
             patch.object(nba_predict_read, "_get_predictions_table"), \
             patch.object(nba_predict_read, "list_events", return_value={}) as mock_list:
            nba_predict_read.lambda_handler(_api_event("/nba/events"), None)

        assert mock_list.call_args.args[-1] == "scheduled"

    def test_models_route(self):
        with patch.object(nba_predict_read, "_get_model_bucket"), \
             patch.object(nba_predict_read, "list_models", return_value={"sport": "nba", "models": []}):
            response = nba_predict_read.lambda_handler(_api_event("/nba/models"), None)
    ...
    def test_unhandled_exception_returns_500(self):
        with patch.object(nba_predict_read, "_get_storage"), \
             patch.object(nba_predict_read, "_get_predictions_table"), \
             patch.object(nba_predict_read, "list_events", side_effect=Exception("boom")):
            response = nba_predict_read.lambda_handler(_api_event("/nba/events"), None)
...
class TestPredictionRoutes:
    EVENT_ID = "401705127"
    EVENT_KEY = build_event_key("nba", EVENT_ID)
    CACHE_KEY = f"predictions-cache/nba/events/{EVENT_KEY}.json"
    EVENT_RESOURCE = "/nba/predictions/events/{event_id}"
    ...
    def test_fresh_cache_hit_returns_the_cached_result_without_triggering_compute(self):
        ...
        state = _model_version_state("nba", versions)
        ...
        with patch.object(nba_predict_read, "_get_model_bucket", return_value=s3), \
             patch.object(nba_predict_read, "_get_predict_invoker") as get_invoker:
            response = nba_predict_read.lambda_handler(
    ...
    def test_stale_cache_hit_returns_203_with_the_cached_result_and_triggers_a_refresh(self):
        ...
        with patch.object(nba_predict_read, "_get_model_bucket", return_value=s3), \
             patch.object(nba_predict_read, "_get_predict_invoker", return_value=invoker):
            response = nba_predict_read.lambda_handler(
                _predict_event(self.EVENT_RESOURCE, {"event_id": self.EVENT_ID}), None,
            )
        ...
        body = json.loads(response["body"])
        assert body == {"ok": "stale", "stale": True, "retry_after_seconds": nba_predict_read.RETRY_AFTER_SECONDS}
    ...
    def test_a_repromoted_model_makes_a_completed_events_own_cache_stale_too(self):
        ...
        current = _model_version_state("nba", {"win_probability": 2, "margin": 1, "home_score": 1, "away_score": 1})
        state = dict(current)
        state[self.CACHE_KEY] = {
    ...
    def test_player_prop_route_missing_stat_returns_400(self):
        response = nba_predict_read.lambda_handler(
            _predict_event(
                "/nba/predictions/events/{event_id}/players/{entity_id}",
    ...
    def test_player_prop_route_fresh_cache_hit(self):
        stat = "points"
        cache_key = f"predictions-cache/nba/events/{self.EVENT_KEY}/players/101/{stat}.json"
        state = {
            current_version_key("nba", "player-prop-points"): {"version": 2},
