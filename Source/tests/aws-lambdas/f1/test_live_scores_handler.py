...
vs. API Gateway's GET /f1/live-scores) and response shaping/error
handling. live_scores.py's own logic is covered directly in
test_live_scores.py -- this file only verifies routing. Same shape as
tests/aws-lambdas/pga/test_live_scores_handler.py.

The f1_live_scores module is registered in sys.modules by conftest.py.
"""
import json
from unittest.mock import MagicMock, patch

import f1_live_scores
import live_scores
...
class TestDispatch:
    ...
    def test_live_scores_route_returns_cached_events(self):
        ...
        assert json.loads(response["body"]) == {"events": {"2026-2": {"state": "in"}}}

    def test_unknown_resource_returns_404(self):
        response = f1_live_scores.lambda_handler(_api_event("/f1/nonsense"), None)
