...
def _make_df(n=10, home_scores=None, away_scores=None):
    return pd.DataFrame({
        ...
        "elo_diff": [10.0] * n,
        "home_avg_points_scored": [110.0] * n,
        "home_avg_points_allowed": [104.0] * n,
        "away_avg_points_scored": [108.0] * n,
        "away_avg_points_allowed": [112.0] * n,
        "label_home_won": [i % 2 == 0 for i in range(n)],
        "label_home_score": home_scores if home_scores is not None else [110] * n,
        "label_away_score": away_scores if away_scores is not None else [104] * n,
    })
...
class TestAddLabel:
    def test_margin_is_home_minus_away(self):
        df = _make_df(3, home_scores=[110, 95, 120], away_scores=[104, 100, 120])
    ...
    def test_home_score_is_the_raw_home_score(self):
        ...
        assert list(result[train_score_model.LABEL_COLUMN]) == [110, 95, 120]
    ...
    def test_away_score_is_the_raw_away_score(self):
        ...
        assert list(result[train_score_model.LABEL_COLUMN]) == [104, 100, 120]
...
class TestNaivePrediction:
    def test_margin_combines_each_teams_own_point_differential(self):
        # home diff = 110-104 = 6, away diff = 108-112 = -4 -> margin = 10
        df = _make_df(3)
    ...
    def test_home_score_averages_home_scoring_with_away_allowing(self):
        ...
        assert list(naive) == [111.0, 111.0, 111.0]
    ...
    def test_away_score_averages_away_scoring_with_home_allowing(self):
        ...
        assert list(naive) == [106.0, 106.0, 106.0]

    def test_missing_history_falls_back_to_the_columns_own_mean_not_zero(self):
        df = _make_df(4)
        df.loc[3, "home_avg_points_scored"] = None  # mean of the other three (110) fills this one

        naive = train_score_model._naive_prediction(df, "home_score")

        assert naive.iloc[3] == pytest.approx((110.0 + 112.0) / 2)
