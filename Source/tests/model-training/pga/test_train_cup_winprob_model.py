"""
Unit tests for the PGA Cup (team) win-probability training entrypoint.

library.ml.backtest.run_backtest is mocked here -- these tests verify
train_cup_winprob_model.py's own orchestration, not the tournament itself
or any real algorithm fitting.
"""
...
import train_cup_winprob_model
...
def _make_df(n=10, home_win_count=5, halved_count=0):
    ...
    return pd.DataFrame({
        ...
        "tournament_name": ["Presidents Cup"] * n,
...
def _fake_result(version=1, algorithm="xgboost"):
    return {
        "promotions": [{"model_name": "cup-win-probability", "algorithm": algorithm, "version": version, "log_loss": 0.6}],
        "candidates": [{"algorithm": algorithm, "log_loss": 0.6}],
    }


class TestFeatureColumns:
    def test_excludes_identifiers_tournament_name_and_every_label_column(self):
        df = _make_df()

        columns = train_cup_winprob_model._feature_columns(df)

        assert columns == ["home_avg_score_to_par", "away_avg_score_to_par"]
...
class TestTrain:
    def test_calls_run_backtest_with_five_candidates_including_lightgbm(self):
        ...
        with patch.object(train_cup_winprob_model.backtest, "run_backtest", return_value=_fake_result()) as mock_run:
            result = train_cup_winprob_model.train(MagicMock(), df)
    ...
    def test_halved_cups_are_dropped_before_the_split(self):
        df = _make_df(10, home_win_count=4, halved_count=2)
...
class TestMain:
    def test_loads_features_and_delegates_to_train(self, monkeypatch):
        ...
        with patch.object(train_cup_winprob_model, "S3Manager", return_value=mock_s3), \
             patch.object(train_cup_winprob_model.training_common, "load_features", return_value=df) as mock_load, \
             patch.object(train_cup_winprob_model, "train", return_value=_fake_result()) as mock_train:
            train_cup_winprob_model.main()

        mock_load.assert_called_once_with(mock_s3, train_cup_winprob_model.CUP_FEATURES_KEY)
