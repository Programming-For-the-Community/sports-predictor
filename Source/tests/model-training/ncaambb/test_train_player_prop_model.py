"""
Unit tests for the NCAA MBB player-prop training entrypoint.
...
NCAA MBB's stat_line has no category-prefixed keys, same as NBA's, so
...
def _make_df(n=10, target_stat="points", missing_stat_rows=0, games_with_stat=None, avg_stat=None):
    ...
    if games_with_stat is None:
        games_with_stat = [5] * n
    if avg_stat is None:
        avg_stat = [15.0 + i for i in range(n)]
    ...
    for i in range(n):
        if i >= n - missing_stat_rows:
            ...
            stat_lines.append(json.dumps({target_stat: 12.0 + i}))
...
class TestFilterToTargetStat:
    ...
    def test_extracts_the_stat_value_as_a_numeric_label(self):
        ...
        assert list(filtered[train_player_prop_model.LABEL_COLUMN]) == [12.0, 13.0, 14.0, 15.0, 16.0]
    ...
    def test_excludes_a_low_volume_recurring_bench_player_relative_to_peers(self):
        # Four rows near a real rotation player's average (15), one row
        # from a deep-bench player whose own average (1) is far below
        # their peers despite clearing games_with_<stat>.
        avg_stat = [15.0, 15.0, 15.0, 15.0, 1.0]
        df = _make_df(5, target_stat="points", games_with_stat=[5] * 5, avg_stat=avg_stat)

        filtered = train_player_prop_model._filter_to_target_stat(df, "points")

        assert len(filtered) == 4
        assert 1.0 not in filtered["avg_points"].values

    def test_keeps_a_player_at_exactly_the_fraction_of_median(self):
        # median([15,15,15,15,5.25]) is 15; 0.35 * 15 = 5.25 exactly.
        avg_stat = [15.0, 15.0, 15.0, 15.0, 5.25]
        df = _make_df(5, target_stat="points", games_with_stat=[5] * 5, avg_stat=avg_stat)
    ...
    def test_median_is_computed_after_the_volume_filter_not_before(self):
        ...
        avg_stat = [15.0, 15.0, 15.0, 3.0, 1.0]
        df = _make_df(5, target_stat="points", games_with_stat=games_with_stat, avg_stat=avg_stat)

        filtered = train_player_prop_model._filter_to_target_stat(df, "points")

        # 4 real rows remain after the volume filter: [15,15,15,3] ->
        # median 15, 0.35*15=5.25 > 3, so the 3.0 row is also excluded.
        assert len(filtered) == 3
        assert 3.0 not in filtered["avg_points"].values
...
class TestFeatureColumns:
    ...
    def test_no_opposing_side_exclusion_a_column_common_to_every_position_survives(self):
        # NCAA MBB has no offense/defense category split, same as NBA --
        # a column survives here purely on MIN_NON_NULL_FRACTION.
...
class TestTrain:
    ...
    def test_naive_baseline_predicts_the_players_own_rolling_average(self):
        n = 10
        df = _make_df(n, target_stat="points", avg_stat=[12.0 + i for i in range(n)])
