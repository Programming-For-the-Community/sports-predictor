"""
Unit tests for the F1 podium-probability training entrypoint.

library.ml.backtest.run_backtest is mocked here -- these tests verify
train_podium_model.py's own orchestration, not the tournament itself or
...
def _fake_result(version=1, algorithm="xgboost"):
    return {
        "promotions": [{"model_name": "podium-probability", "algorithm": algorithm, "version": version, "log_loss": 0.4}],
...
class TestFeatureColumns:
    def test_excludes_identifiers_raw_strings_and_the_label_column(self):
        ...
        columns = train_podium_model._feature_columns(df)
...
class TestTrain:
    def test_calls_run_backtest_with_five_candidates_including_lightgbm(self):
        ...
        with patch.object(train_podium_model.backtest, "run_backtest", return_value=_fake_result()) as mock_run:
            result = train_podium_model.train(MagicMock(), df)
