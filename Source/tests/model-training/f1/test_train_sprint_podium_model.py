"""
Unit tests for the F1 Sprint-podium-probability training entrypoint.

library.ml.backtest.run_backtest is mocked here -- these tests verify
train_sprint_podium_model.py's own orchestration, not the tournament
...
import train_sprint_podium_model


def _make_df(n=10, podium_count=3):
    return pd.DataFrame({
        ...
        "label_podium": [1 if i < podium_count else 0 for i in range(n)],
    })


def _fake_result(version=1, algorithm="xgboost"):
    return {
        "promotions": [{"model_name": "sprint-podium-probability", "algorithm": algorithm, "version": version, "log_loss": 0.4}],
...
class TestFeatureColumns:
    def test_excludes_identifiers_raw_strings_and_the_label_column(self):
        ...
        columns = train_sprint_podium_model._feature_columns(df)
...
class TestTrain:
    def test_calls_run_backtest_with_five_candidates_including_lightgbm(self):
        ...
        with patch.object(train_sprint_podium_model.backtest, "run_backtest", return_value=_fake_result()) as mock_run:
            result = train_sprint_podium_model.train(MagicMock(), df)
    ...
    def test_splits_chronologically(self):
        ...
        with patch.object(train_sprint_podium_model.backtest, "run_backtest", return_value=_fake_result()) as mock_run:
            train_sprint_podium_model.train(MagicMock(), df)
