"""
Unit tests for library.serving.ncaambb_reads -- list_events, list_models,
and their supporting comparison helpers. storage/predictions_table/s3 are
MagicMocks. Byte-for-byte the same shape as test_nba_reads.py --
basketball's leader categories/day-based grouping are identical for both
sports.

No TestRoundLabel here, unlike test_ncaafb_reads.py -- NCAA MBB has no
...
class TestListEvents:
    ...
    def test_completed_returns_only_the_most_recent_date(self):
        ...
        storage.get_all_events.return_value = [
            _event("e1", "2026-01-04", "13", "2", status="completed", home_score=72, away_score=68),
            _event("e2", "2026-01-11", "13", "17", status="completed", home_score=81, away_score=79),
        ]
        storage.get_player_game_stats_for_event.return_value = []

        result = ncaambb_reads.list_events(storage, predictions_table, "ncaambb", "completed")
    ...
    def test_empty_when_nothing_ingested_for_that_status(self):
        storage = MagicMock()
        storage.get_all_events.return_value = []
        result = ncaambb_reads.list_events(storage, MagicMock(), "ncaambb", "scheduled")
    ...
    def test_completed_event_queries_the_predictions_table_exactly_once(self):
        ...
        storage.get_all_events.return_value = [
            _event("e1", "2026-01-11", "13", "2", status="completed", home_score=72, away_score=68),
        ]
        storage.get_player_game_stats_for_event.return_value = []
...
class TestPredictionComparison:
    def test_none_when_game_has_no_final_score(self):
        event = _event("e1", "2026-01-11", "13", "2", status="scheduled")
        assert ncaambb_reads._prediction_comparison(MagicMock(), event) is None

    def test_none_when_no_prediction_was_ever_logged(self):
        predictions_table = MagicMock()
        predictions_table.query.return_value = []
        event = _event("e1", "2026-01-11", "13", "2", status="completed", home_score=72, away_score=68)
        assert ncaambb_reads._prediction_comparison(predictions_table.query.return_value, event) is None
    ...
    def test_compares_predicted_vs_actual(self):
        ...
        predictions_table.query.return_value = [
            ...
            {"model_key": "MODEL#home-score#v1", "predicted_value": {"value": 70.0}},
            {"model_key": "MODEL#away-score#v1", "predicted_value": {"value": 66.0}},
        ]
        event = _event("e1", "2026-01-11", "13", "2", status="completed", home_score=72, away_score=68)

        result = ncaambb_reads._prediction_comparison(predictions_table.query.return_value, event)
...
class TestLeadersComparison:
    def test_none_when_nothing_was_ever_predicted(self):
        predictions_table = MagicMock()
        predictions_table.query.return_value = []
        event = _event("e1", "2026-01-11", "13", "2")
        assert ncaambb_reads._leaders_comparison(MagicMock(), predictions_table.query.return_value, "ncaambb", event) is None
    ...
    def test_scoring_leaders_are_grouped_as_a_list(self):
        ...
        result = ncaambb_reads._leaders_comparison(storage, predictions_table.query.return_value, "ncaambb", event)
...
class TestListModels:
    def test_empty_when_no_models_promoted(self):
        ...
        assert ncaambb_reads.list_models(s3, "ncaambb") == {"sport": "ncaambb", "models": []}

    def test_lists_promoted_models_with_card_summaries(self):
        s3 = MagicMock()
        s3.list_keys.return_value = ["ncaambb/win-probability/v1/model_card.json"]
        ...
        result = ncaambb_reads.list_models(s3, "ncaambb")
...
class TestGetSeasonProjection:
    def test_returns_the_cached_projection_from_its_own_key(self):
        s3 = MagicMock()
        s3.object_exists.return_value = True
        s3.get_json.return_value = {"sport": "ncaambb", "season": 2026, "standings": []}

        result = ncaambb_reads.get_season_projection(s3, "ncaambb")

        assert result == {"sport": "ncaambb", "season": 2026, "standings": []}
        s3.get_json.assert_called_once_with("season-projections/ncaambb/latest.json")

    def test_returns_none_when_the_scheduled_job_hasnt_written_one_yet(self):
        s3 = MagicMock()
        s3.object_exists.return_value = False

        assert ncaambb_reads.get_season_projection(s3, "ncaambb") is None
