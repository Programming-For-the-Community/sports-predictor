...
/// Session restore hasn't finished -- send every route here, not just '/',
/// so the originally-requested route's real content never builds and
/// flashes on screen before this redirect resolves.
...
/// Router-level branch -- keeps EventListPage/EventDetailPage/GameRow
/// completely untouched, mirroring how SportCard already branches on
/// eventShape at the leaf-widget level (sport_card.dart). F1 is checked by
/// id, not just eventShape -- it shares EventShape.field with PGA (both
/// render a ranked list, never W/L), but the two sports' own /events and
/// /predictions response shapes are genuinely different (f1_event.dart/
/// f1_prediction.dart vs field_event.dart/field_prediction.dart), so
/// eventShape alone can't pick the right widget the way it does for
/// headToHead vs field.
Widget _eventListPageFor(String sportId) {
...
Widget _eventDetailPageFor(String sportId, String eventId) {
...
Widget _seasonPageFor(String sportId) {
  ...
  return GoRouter(
    ...
    routes: [
      ...
      ShellRoute(
        ...
        routes: [
          GoRoute(
            // Same path template regardless of sport, so this can't be
            // three separate GoRoutes -- see _eventListPageFor's own doc.
            path: '/:sport/events',
            builder: (context, state) => _eventListPageFor(state.pathParameters['sport']!),
            routes: [
              GoRoute(
                path: ':eventId',
                builder: (context, state) => _eventDetailPageFor(
          ...
          GoRoute(
            path: '/:sport/season',
            builder: (context, state) => _seasonPageFor(state.pathParameters['sport']!),
          ),
        ],
      ),
    ],
