...
# Basketball's own leader categories -- identical for nba/ncaambb (scoring/
# rebounding/assists, one stat each). Matches library.serving.{sport}_reads'
# own _STAT_CATEGORY, inverted. ncaafb/nfl each have their own genuinely
# different category shape (multi-stat categories, a singular "passing"
# leader) and keep predict_event_leaders fully local for that reason.
BASKETBALL_LEADER_CATEGORY_STATS = {
    "scoring": ["points"],
    "rebounding": ["rebounds"],
    "assists": ["assists"],
}
...
def _score_and_record_leader(
    storage, s3, predictions_table, sport: str, model_cache: dict, event_key_value: str,
    feature_row: dict, stats: list[str],
) -> dict:
    ...
    for stat in stats:
        ...
        try:
            booster, model_card = get_cached_model(model_cache, s3, sport, model_name)
...
def basketball_predict_event_leaders(
    storage, s3, predictions_table, event_key_value: str, sport: str, events: list[dict] | None = None,
) -> dict | None:
    """The `leaders` block for nba/ncaambb -- scoring/rebounding/assists
    leaders per team, each always a list (no leader is inherently singular
    in basketball). Best-effort: a failure here is logged and returns None
    rather than failing predict_event."""
    try:
    ...
    def score(feature_row: dict, stats: list[str]) -> dict:
        return _score_and_record_leader(storage, s3, predictions_table, sport, model_cache, event_key_value, feature_row, stats)
    ...
    def team_leaders(team_candidates: dict) -> dict:
        ...
        result = {}
        for category in BASKETBALL_LEADER_CATEGORY_STATS:
            primary_stat = BASKETBALL_LEADER_CATEGORY_STATS[category][0]
            scored = [score(row, BASKETBALL_LEADER_CATEGORY_STATS[category]) for row in team_candidates[category]]
