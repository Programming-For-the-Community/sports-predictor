"""
NCAA MBB player-prop model training -- one model per target stat (e.g.
TARGET_STAT=points, TARGET_STAT=rebounds). Reads player_features.parquet
(written by Source/feature-engineering/ncaambb/build_dataset.py's
...
library/features/ncaambb.py and library/features/common.py). Run a given
stat via the TARGET_STAT environment variable at `aws ecs run-task` time.

Every NCAA MBB player's stat_line carries the same flat key set (points,
rebounds, assists, steals, blocks, turnovers,
field_goals_made/field_goal_attempts, etc.) regardless of position, same
as NBA's own box score -- MIN_NON_NULL_FRACTION alone is sufficient in
_feature_columns, no side-of-the-ball exclusion to apply.
...
def _feature_columns(df: pd.DataFrame) -> list[str]:
    """MIN_NON_NULL_FRACTION alone is the filter; NCAA MBB's flat
    stat_line has no side-of-the-ball distinction to exclude."""
