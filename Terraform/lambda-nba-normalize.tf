...
# schema (splitting ESPN's combined "made-attempted" stat strings), and
# upserts entities/events/player_game_stats/team_game_stats. No ESPN
# calls of its own (transforms JSON already fetched by ingest), so it
...
resource "aws_lambda_function" "nba_normalize" {
  ...
  # 300s/1024MB -- generous headroom for a recurring nightly job. NBA's
  # ingest writes one S3 object per game (~26 players) and per team
  # roster (~15-17 players), a small enough per-invocation volume that a
  # plain serial loop is sufficient.
  timeout     = 300
  memory_size = 1024
