...
resource "aws_cloudwatch_log_group" "f1_train_sprint_podium_model" {
  name              = "/ecs/${var.project}-f1-train-sprint-podium-model"
...
# train_sprint_podium_model.py, same shared-image/command-override
...
resource "aws_ecs_task_definition" "f1_train_sprint_podium_model" {
  family                   = "${var.project}-f1-train-sprint-podium-model"
  ...
  container_definitions = jsonencode([
    {
      name      = "f1-train-sprint-podium-model"
      image     = "${var.ecr_repo_url}:f1-train-winprob-model-latest"
      command   = ["train_sprint_podium_model.py"]
      essential = true
      environment = [
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          "awslogs-group"         = aws_cloudwatch_log_group.f1_train_sprint_podium_model.name
          "awslogs-region"        = var.region
          "awslogs-stream-prefix" = "train-sprint-podium-model"
        }
      }
    }
  ])
