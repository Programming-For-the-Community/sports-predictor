...
#
# Monday 14:00 UTC -- distinct from every other sport's own weekly
# season-projection schedule (NBA Fri, NCAAFB Thu, NFL Wed, NCAAMBB daily),
# and lands Mon-Wed rather than Thu-Sun, which is when a PGA Tour event is
# actually being played. aws_iam_role.eventbridge_invoke already covers
...
resource "aws_scheduler_schedule" "pga_season_projection" {
  ...
  description = "Invokes the pga_predict Lambda weekly, Mon 14:00 UTC, to recompute the FedEx Cup season simulation and cache it to S3 for GET /pga/season."
