...
# Standalone Fargate task for the National Ranking (1-25) model,
# team-week-level rather than event-level or player-level. Trained only on
# ranked-team-weeks (CFBD AP Poll rank as the label); at serving time every
# FBS team's predicted score is sorted and the lowest 25 become the
# projected Top 25.
#
# Its own image, not the win-probability task's -- team-week feature
