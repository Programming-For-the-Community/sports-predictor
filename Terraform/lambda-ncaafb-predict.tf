...
resource "aws_lambda_function" "ncaafb_predict" {
  ...
  description   = "Computes NCAAFB event-outcome, player-prop, and ranking predictions in the background. Never called by API Gateway -- invoked by EventBridge Scheduler or an async invoke from the shared predict-read Lambda on a cache miss. See predict/handler.py."
