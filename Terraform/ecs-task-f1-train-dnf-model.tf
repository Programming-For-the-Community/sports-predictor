...
resource "aws_cloudwatch_log_group" "f1_train_dnf_model" {
  name              = "/ecs/${var.project}-f1-train-dnf-model"
...
# train_dnf_model.py, same shared-image/command-override pattern PGA's
# own train-top5-model task definition uses.
resource "aws_ecs_task_definition" "f1_train_dnf_model" {
  family                   = "${var.project}-f1-train-dnf-model"
  ...
  container_definitions = jsonencode([
    {
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          "awslogs-group"         = aws_cloudwatch_log_group.f1_train_dnf_model.name
          "awslogs-region"        = var.region
          "awslogs-stream-prefix" = "train-dnf-model"
        }
      }
    }
  ])
