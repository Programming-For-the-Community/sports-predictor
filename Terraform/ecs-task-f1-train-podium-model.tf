...
resource "aws_cloudwatch_log_group" "f1_train_podium_model" {
  name              = "/ecs/${var.project}-f1-train-podium-model"
...
# train_podium_model.py, same shared-image/command-override pattern PGA's
...
resource "aws_ecs_task_definition" "f1_train_podium_model" {
  ...
  container_definitions = jsonencode([
    {
      name      = "f1-train-podium-model"
      image     = "${var.ecr_repo_url}:f1-train-winprob-model-latest"
      command   = ["train_podium_model.py"]
      essential = true
      environment = [
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          "awslogs-group"         = aws_cloudwatch_log_group.f1_train_podium_model.name
          "awslogs-region"        = var.region
          "awslogs-stream-prefix" = "train-podium-model"
        }
      }
    }
  ])
