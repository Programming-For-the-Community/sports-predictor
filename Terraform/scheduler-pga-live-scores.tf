# Invokes the pga_live_scores Lambda every 5 minutes to refresh its
# leaderboard-snapshot cache. Runs unconditionally, every 5 minutes,
...
resource "aws_scheduler_schedule" "pga_live_scores" {
  ...
  description = "Invokes the pga_live_scores Lambda every 5 minutes to refresh its leaderboard-snapshot cache for any tournament in its daily poll window (1h before the day's tee times through 1h after the last active golfer's status went non-active)."
