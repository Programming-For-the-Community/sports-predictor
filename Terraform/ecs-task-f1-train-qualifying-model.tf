...
resource "aws_cloudwatch_log_group" "f1_train_qualifying_model" {
  name              = "/ecs/${var.project}-f1-train-qualifying-model"
...
# train_qualifying_model.py, same shared-image/command-override pattern
# PGA's own train-top5-model task definition uses.
resource "aws_ecs_task_definition" "f1_train_qualifying_model" {
  family                   = "${var.project}-f1-train-qualifying-model"
  ...
  container_definitions = jsonencode([
    {
      name      = "f1-train-qualifying-model"
      image     = "${var.ecr_repo_url}:f1-train-winprob-model-latest"
      command   = ["train_qualifying_model.py"]
      essential = true
      environment = [
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          "awslogs-group"         = aws_cloudwatch_log_group.f1_train_qualifying_model.name
          "awslogs-region"        = var.region
          "awslogs-stream-prefix" = "train-qualifying-model"
        }
      }
    }
  ])
