...
resource "aws_cloudwatch_log_group" "f1_train_finish_position_model" {
  name              = "/ecs/${var.project}-f1-train-finish-position-model"
...
# train_finish_position_model.py, same shared-image/command-override
...
resource "aws_ecs_task_definition" "f1_train_finish_position_model" {
  ...
  container_definitions = jsonencode([
    {
      name      = "f1-train-finish-position-model"
      image     = "${var.ecr_repo_url}:f1-train-winprob-model-latest"
      command   = ["train_finish_position_model.py"]
      essential = true
      environment = [
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          "awslogs-group"         = aws_cloudwatch_log_group.f1_train_finish_position_model.name
          "awslogs-region"        = var.region
          "awslogs-stream-prefix" = "train-finish-position-model"
        }
      }
    }
  ])
